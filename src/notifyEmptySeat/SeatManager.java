...
public class SeatManager {
	private Map<String, SeatCourse> previousCourses = new HashMap<>(); // 바뀌기전과목 상태를 저장해놓기
	...
	public SeatManager(String filePath) {
		...
		for (SeatCourse course : previousCourses.values()) { //
	...
	public Map<String, SeatCourse> readCourseFile() {
		Map<String, SeatCourse> courses = new HashMap<>();
		...
		try (BufferedReader br = new BufferedReader(new FileReader(filePath))) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (tokens.length >= 17) { // 임시 인풋 텍스트에서 정보를 17개로 정함
					...
					SeatCourse course = new SeatCourse(courseId, courseName, maxStudents, currentStudents);
	...
	public synchronized void checkChanges() {
		Map<String, SeatCourse> currentCourses = readCourseFile(); // 최신파일 읽기

		// 최신파일이랑 옛날 정보랑 비교
		for (SeatCourse cur : currentCourses.values()) {
			SeatCourse prev = previousCourses.get(cur.getCourseId()); // 옛날 과목코드 조회
